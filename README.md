# 🌏 地震監視システム v3.3 - TV同等津波監視版（気象庁70地域GeoJSON + P2Pライブ連携）

## 概要

**TV同等の津波リアルタイム表示**を実現した地震・津波監視システム。気象庁公式 70 地域の津波予報区 GeoJSON を採用し、P2P 気象庁情報(code 552)を WebSocket / REST で受信して実データで警報レベル別に着色・点滅させます。v3.2 で確立した CSP 対応・XSS 対策・外部スクリプトファイル化などのセキュリティ強化も継承しています。
